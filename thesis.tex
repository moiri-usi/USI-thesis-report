...
\begin{itemize}
    ...
    \item $ \theta_{ij, r} $, the parameters of the kernel $ r $, with
        $i,j \in \{1, \dots, N\} $
    \item $v_{ij} $, forming the extended transition probability matrix $V$ of
        size $N \times N$
    ...
    \item $\kappa_{ij}(d_k|\theta_{ij})$ forming the kernel matrix of size
        $N \times N$
    \item $\alpha_k(i)$: i-th component of the forward variable vector
        $\alpha_k$ with $k \in \{1, \dots, L\}$
    \item $O_k$: k-th observation element of the observation sequences
        $O_1, \dots, O_L$
    \item $o_k$: k-th observation symbol of the set of observation symbols
        $\{o_1, \dots, o_M\}$
